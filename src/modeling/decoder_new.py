    ...
    def run_set_predict(self, goals_2D, scores, mapping, device, loss, i):
        ...
        if args.do_train:
            ...
            if True:
                ...
                #input = torch.cat([predicts[min_cost_idx].reshape(1,12), dynamic_label.reshape(1,12)], dim = 0)
                input = torch.cat([predicts[min_cost_idx].reshape(1,12), torch.zeros([1,12]), device = device), dim = 0)
                #print("input:", input)
